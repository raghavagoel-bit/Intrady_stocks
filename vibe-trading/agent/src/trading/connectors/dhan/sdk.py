...
def get_historical_bars(
    ...
    *,
    ...
    try:
        if period in ("1m", "5m", "15m", "30m"):
            if hasattr(client, "intraday_minute_data"):  # dhanhq >= 2.x
                data = client.intraday_minute_data(
                    security_id=sec_id,
                    exchange_segment=segment,
                    instrument_type=instrument_type,
                    from_date=from_date.strftime("%Y-%m-%d"),
                    # Dhan v2 treats toDate as exclusive for intraday candles —
                    # +1 day so the current session's bars are included.
                    to_date=(to_date + timedelta(days=1)).strftime("%Y-%m-%d"),
                    interval=int(period.rstrip("m")),
                )
            else:  # dhanhq 1.x
            ...
            if hasattr(client, "historical_daily_data"):  # dhanhq >= 2.x
    ...
    bars = []
    if isinstance(payload, dict) and payload.get("candles") is not None:
        # v1 shape: data.candles = [[time, o, h, l, c, v], ...]
        for candle in _as_list(payload.get("candles")):
            if len(candle) >= 6:
                bars.append({
                    "time": candle[0],
                    "open": candle[1],
    ...
    elif isinstance(payload, dict) and payload.get("timestamp") is not None:
        # v2 shape: parallel arrays — timestamp/open/high/low/close/volume.
    ...
    if not bars and str(data.get("status", "")).lower() == "failure":
        return {
            "status": "error",
            "error": str(data.get("remarks") or "Dhan returned failure"),
            "symbol": symbol,
        }
...
def _dhan_client(cfg: DhanConfig):
    _require_dhan()
    import dhanhq as _dhanhq  # type: ignore
    ...
    # dhanhq >= 2.1 wraps creds in a DhanContext; 1.x takes them positionally.
    DhanContext = getattr(_dhanhq, "DhanContext", None)
